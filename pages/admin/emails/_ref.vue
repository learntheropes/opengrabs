...
export default {
    ...
    async asyncData({ app, modalWidth, params: { ref }}) {
        const [isAdmin, ticket, messages] = await Promise.all([
            ...
            app.$admin.tickets.email.messages.filter(ref, modalWidth)
        ])
            ...
            if (validMessage) {
                ...
                if (this.attachments.length) {
                    for (const attachment of this.attachments) {
                        ...
                        fd.append('folder', `${process.env.BTC_CHAIN}/email_tickets`)
                        fd.append('overwriteFile', false)
                        fd.append('tags', `${this.ticket},${this.me}`)
                ...
                this.messages = await this.$admin.tickets.email.messages.filter(this.ref, this.modalWidth) 
