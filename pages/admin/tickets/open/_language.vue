<template>
    <section v-if="isAdmin" class="section">
        <div class="columns">
            ...
            <div class="column">
                <h1 v-if="language === 'en'" class="title">English open tickets</h1>
                <h1 v-if="language === 'es'" class="title">Spanish open tickets</h1>
                <h1 v-if="language === 'pt'" class="title">Portuguese open tickets</h1>
                <h1 v-if="language === 'ru'" class="title">Russian open tickets</h1>
...
export default {
    ...
    async asyncData({ app, params: { language }}) {
        const [isAdmin, tickets] = await Promise.all([
            app.$admin.isAdmin(),
            app.$admin.tickets.filter('open', language) 
        ])
